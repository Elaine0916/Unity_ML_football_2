...
public class Robort : Agent
{
    ...
    private void Start()
    {
        rigRobort = GetComponent<Rigidbody>();
    ...
    {
        //重設剛體加速度與角度加速度
        rigRobort.velocity = Vector3.zero;
        rigRobort.angularVelocity = Vector3.zero;
        rigBall.velocity = Vector3.zero;
        rigBall.angularVelocity = Vector3.zero;

        //隨機機器人位置
        Vector3 posRobort = new Vector3(Random.Range(-1f, 1f), 0.1f, Random.Range(-2f, 0f));
        transform.position = posRobort;
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        sensor.AddObservation(rigRobort.velocity.x);
        sensor.AddObservation(rigRobort.velocity.z);
    }
    ...
    public override void OnActionReceived(float[] vectorAction)
    {
        ...
        rigRobort.AddForce(control * speed);
